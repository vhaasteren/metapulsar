...
* Flags include `pta`, `pta_dataset`, and `timing_package` tags for each TOA, and may also include `mjd_jump_pta` when the release par has `JUMP MJD` windows and `canonicalize_tim=True`. When `canonicalize_tim=True`, those flags are read back from the canonical `.tim` the factory wrote (see “Canonical `.tim` artifacts” below). When a MetaPulsar is built with `canonicalize_tim=False`, or directly from pulsar objects, `MetaPulsar._combine_flags()` fills only the PTA metadata flags (`pta` / `pta_dataset` / `timing_package`) for legs that lack them.

Concatenation itself does not alter TOA values; any `TIME` baking already happened when each leg’s canonical `.tim` was written (`canonicalize_tim=True`).

### Canonical `.tim` artifacts

By default (`canonicalize_tim=False`) every PTA leg loads the release `.tim` path in place, so relative `INCLUDE`s resolve in the release tree; the tree is not copied or relocated. Pulse-number derivation, when needed, persists only its generated standalone `.tim`. Pass `canonicalize_tim=True` to load from a rewritten standalone Tempo2 `FORMAT 1` `.tim` instead, so the PTA identity of a TOA travels with the data instead of living only in memory. The rewrite is dual-engine-reloadable. The AEI-DR2/DR3 rebuild scripts opt in explicitly; without it, cross-engine `TIME`/`INCLUDE` parity and `-mjd_jump_pta` stamps are not provided, and `convert_jump_mjd=True` is refused. With the opt-in:
...
* `-pn` pulse numbers are present when `use_pulse_numbers` asks for them; that setting is independent of `canonicalize_tim`.
* `convert_jump_mjd=False` (default) leaves engine-par `JUMP MJD` lines intact while still stamping the tim flags when canonicalizing. `convert_jump_mjd=True` rewrites each engine-par line to `JUMP -mjd_jump_pta {pta}_{k} …` with the same values and requires `canonicalize_tim=True`; release `.par` files on disk are never mutated.

`TIME` ownership matches the parsing leg: tempo2 is file-local per `INCLUDE`; PINT shares one accumulator across includes. Flattening is refused (with an error, never a silent shift) if tempo2 stateful directives are live at an `INCLUDE` boundary. With `canonicalize_tim=True`, passing `timfile_output_dir` exports the standalone canonical engine inputs so a combination can be reproduced or handed to other tooling directly. False mode rejects `timfile_output_dir` because release TIM trees are loaded in place and are not exported.
...
* **Flags and metadata.** With `canonicalize_tim=True`, `metapulsar.tim_canonical.write_canonical_tim(...)` stamps `pta`, `pta_dataset`, and `timing_package` into the `.tim` each engine loads, and `-mjd_jump_pta` on TOAs selected by release `JUMP MJD` windows, so the combined flags are read back from the data. `MetaPulsar._combine_flags()` fills the PTA metadata flags in only for legs that lack them (`canonicalize_tim=False`, or a MetaPulsar built directly from pulsar objects). Planetary and positional arrays are copied row‑wise from the materialized PTA timing records.
