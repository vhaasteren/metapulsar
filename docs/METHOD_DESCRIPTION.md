...
With `canonicalize_tim=True` (default) every PTA leg is loaded from a rewritten standalone Tempo2 `FORMAT 1` `.tim` rather than the release file, so the PTA identity of a TOA travels with the data instead of living only in memory. The rewrite is dual-engine-reloadable. Pass `canonicalize_tim=False` to skip it and load the release `.tim` tree (escape hatch when flattening refuses a published release; cross-engine `TIME`/`INCLUDE` parity and `-mjd_jump_pta` stamps are then not provided; `convert_jump_mjd=True` is refused):
